...
LANGUAGE_MAP_DIR=${LANGUAGE_MAP_DIR:-"${REPO_ROOT}/tools/two_stage_clustering"}
...
COLA_TIER3_SAMPLE_DIR=${COLA_TIER3_SAMPLE_DIR:-/scratch/hpc-prf-merlin/project_data/moe_study/adapter_dataset/cola_200_tier_samples/sharded_samples}
...
COLA_TIER3_EXT_TOKENIZER=${COLA_TIER3_EXT_TOKENIZER:-/scratch/hpc-prf-merlin/project_data/moe_study/tokenizer_extension/cola_tier3/extended_tokenizer}
...
for tier in "${TIER_ORDER[@]}"; do
  ...
  language_map="${TIER_LANGUAGE_MAPS[${tier}]:-}"
  if [[ -z "${language_map}" ]]; then
    echo "[ERROR] No language_map defined for ${tier}" >&2
    exit 1
  fi

  base_output="${OUTPUT_BASE}/${BASE_TOKENIZER_SLUG}/${tier}"
  base_job="${JOB_PREFIX_BASE}_${tier}_orig"
  submit_pipeline "${shard_dir}" "${BASE_TOKENIZER_NAME}" "${base_output}" "${base_job}" "${num_ranks}" "${num_proc}" "${language_map}"
  ...
  submit_pipeline "${shard_dir}" "${ext_tokenizer}" "${ext_output}" "${ext_job}" "${num_ranks}" "${num_proc}" "${language_map}"
done
