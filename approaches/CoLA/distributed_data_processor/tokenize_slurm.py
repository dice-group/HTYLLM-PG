...
def _assign_shards(shards: List[Path], rank: int, world_size: int) -> List[Path]:
    if world_size <= 1:
        return shards
...
def _iter_shard_lines(shards: List[Path]) -> Iterator[dict]:
...
def main(args):
    ...
    with tempfile.TemporaryDirectory(prefix="tok_simple_") as cache_dir:
        raw_dataset = Dataset.from_generator(
            lambda: _iter_shard_lines(assigned),
            cache_dir=cache_dir,
        )
