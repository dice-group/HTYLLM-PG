...
def _setup_cola_tuning(
    ...
    if model_args.adapter_name_or_path is not None:
        ...
        for adapter in adapter_to_merge:
            model: "ColaModel" = PeftModel.from_pretrained(model, adapter, **init_kwargs)
            # model = model.merge_and_unload()
...
def _setup_prompt_tuning(
    ...
    if is_trainable:
        logger.info_rank0("Fine-tuning method: Prompt Tuning")
    ...
    if model_args.adapter_name_or_path is not None:
        ...
        for adapter in adapter_to_merge:
            model: "PromptEmbedding" = PeftModel.from_pretrained(model, adapter, **init_kwargs)
            # model = model.merge_and_unload()
    ...
    prompt_tuning_init_text = "Answer the following question as required.\n"
...
def _setup_p_tuning(
    ...
    if is_trainable:
        logger.info_rank0("Fine-tuning method: P-Tuning")
    ...
    if model_args.adapter_name_or_path is not None:
        ...
        for adapter in adapter_to_merge:
            model: "PromptEncoder" = PeftModel.from_pretrained(model, adapter, **init_kwargs)
            # model = model.merge_and_unload()
...
def _setup_ia3_tuning(
    ...
    if is_trainable:
        logger.info_rank0("Fine-tuning method: IA3 Tuning")
    ...
    if model_args.adapter_name_or_path is not None:
        ...
        for adapter in adapter_to_merge:
            model: "IA3Model" = PeftModel.from_pretrained(model, adapter, **init_kwargs)
            # model = model.merge_and_unload()
...
def init_adapter(
    ...
    elif finetuning_args.finetuning_type == "prompt_tuning":
        model = _setup_prompt_tuning(
            config, model, model_args, finetuning_args, is_trainable, cast_trainable_params_to_fp32
        )
    elif finetuning_args.finetuning_type == "ia3":
        model = _setup_ia3_tuning(
            config, model, model_args, finetuning_args, is_trainable, cast_trainable_params_to_fp32
        )
