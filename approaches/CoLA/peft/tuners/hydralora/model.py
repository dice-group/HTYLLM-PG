...
class HydraLoraModel(BaseTuner):
    """
    ...
    Args:
        ...
        config ([`HydraLoraConfig`]): The configuration of the HydraLora model.
    ...
    Example:
        ...
        >>> from peft import HydraLoraModel, HydraLoraConfig
        ...
        >>> lora_model = HydraLoraModel(model, config, "default")
        ```

        ```py
        >>> import torch
        >>> import transformers
        >>> from peft import HydraLoraConfig, PeftModel, get_peft_model, prepare_model_for_kbit_training
    ...
    **Attributes**:
        ...
        - **peft_config** ([`HydraLoraConfig`]): The configuration of the Lora model.
    """
    ...
    def _check_new_adapter_config(self, config: HydraLoraConfig) -> None:
        """
    ...
    def _prepare_model(self, peft_config: HydraLoraConfig, model: nn.Module):
        r"""
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, HydraLoraLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def set_adapter(self, adapter_name: str | list[str]) -> None:
        ...
        """
        for module in self.model.modules():
            if isinstance(module, HydraLoraLayer):
                if module.merged:
