...
- Fix (in progress): When the FSDP accelerate config is selected, the launcher forces `pure_bf16=true` and `_setup_cola_tuning` now explicitly casts every trainable adapter parameter to bf16, ensuring each FSDP-wrapped module sees a uniform dtype before flattening.
