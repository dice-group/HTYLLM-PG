...
# import tiktoken  # Remove tiktoken import since we're using character-level encoding
import torch
...
if __name__ == '__main__':
    ...
    # Character-level encoding implementation
    # Get all unique characters from the text
    chars = sorted(list(set(text)))
    vocab_size = len(chars)
    print(f"Character vocabulary size: {vocab_size}")
    
    # Create mapping from character to integer and vice versa
    ...
    with open('char_vocab.txt', 'w', encoding='utf-8') as f:
        for char in chars:
            f.write(f"{char}\n")
    print(f"Character vocabulary saved to char_vocab.txt")
    
    # Print a sample of the text for verification
    print("Sample of dataset text:")
    print(text[:1000])
    print(f"Sample encoding: {encode(text[:20])}")
    print(f"Sample decoding: {decode(encode(text[:20]))}")
    
    # Initialize model configuration
    model_config = GPTConfig(
        block_size=block_size,
        vocab_size=vocab_size,  # Now using the character vocabulary size
        n_layer=n_layer,
        n_head=n_head,
        n_embd=n_embd,
        dropout=dropout,
    ...
    if args.load_model:
        ...
        try:
            ...
            if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
                ...
                # Also load vocabulary if available in the checkpoint
                if 'char_to_ix' in checkpoint:
                    char_to_ix = checkpoint['char_to_ix']
                    ix_to_char = {i: ch for ch, i in char_to_ix.items()}
                    encode = lambda s: [char_to_ix.get(c, 0) for c in s]  # Default to 0 for unknown chars
                    decode = lambda l: ''.join([ix_to_char.get(i, ' ') for i in l])
                    print("Character vocabulary loaded from checkpoint")
            else:
                # Load from direct state dict
                model.load_state_dict(checkpoint)
                print("Successfully loaded model state dict")
                # Note: In this case, we assume the vocabulary is the same
    ...
    if args.inference_only:
        ...
        # For inference, ensure we have the correct character mapping
        if not os.path.exists('char_vocab.txt') and 'char_to_ix' not in locals():
            print("Error: No character vocabulary found for inference")
            exit(1)
        
        if 'char_to_ix' not in locals():
            ...
            encode = lambda s: [char_to_ix.get(c, 0) for c in s]  # Default to 0 for unknown chars
            decode = lambda l: ''.join([ix_to_char.get(i, ' ') for i in l])
            print("Character vocabulary loaded from file")
        ...
        if iter % eval_interval == 0 or iter == max_iters - 1:
            ...
            if losses['val'] < best_val_loss:
                ...
                torch.save({
                    ...
                    'char_to_ix': char_to_ix,  # Save vocabulary mapping with model
                }, 'gpt_2_char_enc_best.pt')
    ...
    model_save_path = 'gpt_2_char_enc_final.pt'
    torch.save({
        'model_state_dict': model.state_dict(),
        'char_to_ix': char_to_ix,  # Save vocabulary mapping with model
